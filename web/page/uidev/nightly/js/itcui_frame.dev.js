/*packaged at 2014-07-31 10:40:13*/
/*
...
_ITC._switchTab = function(id){	
	...
	//切换树时还需要保持半隐状态
	if(!_ITC.noTree){
		if(_ITC.opts.treeDisplayState=="ALWAYS_OFF" || _ITC.specTreeState[id]=="fold"){
...
_ITC.init = function(opts){
	...
	$("<span class='tree-btn tree-hide-btn tree-bottom' onclick='_ITC.mannalHideTree()'><span class='tree-btn-left' id='btn_treefold'></span></span>").appendTo($("#" + opts.treeId)).click(function(e){e.stopPropagation()});
...
_ITC._halfDisplayTree = function(state){
	if(state){
		$("#btn_treefold").addClass("tree-btn-right");
		$("#" + _ITC.opts.treeId).addClass("tree-dark");
		$("#" + _ITC.opts.treeId).children(".itc_navtree").addClass("half-hide");
		$("#" + _ITC.opts.bottomId).addClass("tree-width-p").addClass("tree-half-p");
	}
	else{
		$("#" + _ITC.opts.treeId).removeClass("tree-dark");
		$("#btn_treefold").removeClass("tree-btn-right");
...
_ITC.halfDisplayTree = function(state,noAnimate){	
	...
	if(!noAnimate){
		...
		if(state){
			...
			$("#" + _ITC.opts.treeId).animate({"width" : _ITC.opts.treeWidth + "px"});
			_ITC._halfDisplayTree(state);
		}
	}
